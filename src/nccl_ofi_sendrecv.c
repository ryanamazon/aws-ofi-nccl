...
static inline struct fid_domain* get_domain_from_endpoint(nccl_net_ofi_ep_t *base_ep)
{
	nccl_net_ofi_sendrecv_ep_t *ep =
					      ...
					      size_t size, int type, void **mhandle)
{
	...
	domain = get_domain_from_endpoint(base_comm->ep);
							    ...
							    char *remote_ep_addr)
{
	...
	domain = get_domain_from_endpoint(&ep->base);
